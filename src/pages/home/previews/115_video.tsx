...
const Preview = () => {
  ...
  onMount(async () => {
    ...
    handleResp(resp, (data) => {
      const sources = data.filter((s) => s.url)
      if (sources.length === 0) {
        notify.error("No official play source found")
        return
      }
      // array is already highest-quality first
      option.url = sources[0].url
      option.quality = sources.map((s, i) => {
        return {
          html: s.resolution,
          url: s.url,
          default: i === 0,
      ...
      // 115 play urls may be .m3u8 (HLS) or direct mp4; only force m3u8 type
      // when the chosen url is HLS, otherwise let Artplayer auto-detect mp4
      if (sources[0].url.includes(".m3u8")) {
        option.type = "m3u8"
      } else {
        delete option.type
      }
  ...
  async function resetPlayUrl() {
    ...
    handleResp(resp, async (data) => {
      ...
      await player.switchUrl(sources[0].url)
